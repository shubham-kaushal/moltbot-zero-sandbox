...
export interface MoltbotEnv {
  ...
  ANTHROPIC_CUSTOM_URL?: string; // Anthropic-compatible base URL → default_provider = "anthropic-custom:<url>"
  CUSTOM_API_KEY?: string; // API key for custom/anthropic-custom providers (alias: ZEROCLAW_API_KEY)
  ZEROCLAW_API_KEY?: string; // Generic API key read directly by zeroclaw
  CUSTOM_PROVIDER_API?: string; // "openai-chat-completions" or "openai-responses" (custom: only)
  ZEROCLAW_RESPONSES_WEBSOCKET?: string; // "0" disables websocket mode, "1" forces it
  DEFAULT_MODEL?: string; // Override default model for any provider
