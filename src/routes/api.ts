...
adminApi.get('/devices', async (c) => {
  ...
  try {
    ...
    // Run ZeroClaw CLI to list channels/devices
    const proc = await sandbox.startProcess('zeroclaw channel list');
...
adminApi.post('/devices/:requestId/approve', async (c) => {
  ...
  try {
    ...
    // Run ZeroClaw CLI to approve the device
    // TODO: confirm zeroclaw channel approve command and flags
    const proc = await sandbox.startProcess(`zeroclaw channel approve ${requestId}`);
    await waitForProcess(proc, CLI_TIMEOUT_MS);
...
adminApi.post('/devices/approve-all', async (c) => {
  ...
  try {
    ...
    const listProc = await sandbox.startProcess('zeroclaw channel list');
...
adminApi.post('/storage/sync', async (c) => {
  ...
  try {
    ...
    const errorMessage = error instanceof Error ? error.message : 'Unknown error';
    console.error('[storage/sync] Unexpected error:', error);
...
adminApi.post('/gateway/restart', async (c) => {
  ...
  try {
    ...
    // Clean up inside the container: remove startup lock and kill any remaining
    // zeroclaw processes. This ensures the new start-zeroclaw.sh doesn't hit the
    // lock guard (the bash PID in the lock file may still be alive even after kill()).
    try {
      const cleanup = await sandbox.startProcess(
        "bash -c \"rm -f /tmp/.zeroclaw-startup.lock; pkill -f 'zeroclaw gateway' 2>/dev/null; pkill -f 'zeroclaw daemon' 2>/dev/null; sleep 1\"",
      );
      await new Promise((r) => setTimeout(r, 2000));
      console.log('Cleanup process id:', cleanup.id);
    } catch (cleanupErr) {
      console.error('Cleanup step failed (non-fatal):', cleanupErr);
    }
