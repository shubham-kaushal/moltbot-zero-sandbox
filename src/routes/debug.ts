...
debug.get('/version', async (c) => {
  ...
  try {
    // Get ZeroClaw version
    const versionProcess = await sandbox.startProcess('zeroclaw --version');
...
// GET /debug/cli - Test ZeroClaw CLI commands
debug.get('/cli', async (c) => {
  const sandbox = c.get('sandbox');
  const cmd = c.req.query('cmd') || 'zeroclaw --help';
...
// GET /debug/container-config - Read the zeroclaw config from inside the container
debug.get('/container-config', async (c) => {
  const sandbox = c.get('sandbox');

  try {
    const proc = await sandbox.startProcess('cat /root/.zeroclaw/config.toml');
